...
def test_adk_close_widget():
    ctx = hubscape_adk.RemoteContext(
        user_id="user123",
        agent_id="custom-agent"
    )
    res = ctx.close_widget(message_id="msg_999", result_text="✅ Task widget closed.")
    assert res["status"] == "success"
    assert len(ctx.actions) == 1
    assert ctx.actions[0]["type"] == "CLOSE_AGENT_WIDGET"
    assert ctx.actions[0]["payload"]["messageId"] == "msg_999"
    assert ctx.actions[0]["payload"]["resultText"] == "✅ Task widget closed."
